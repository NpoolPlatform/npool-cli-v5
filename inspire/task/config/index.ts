...
import {
  ...
  UserGetTasksRequest,
  UserGetTasksResponse,
  TaskConfig
} from './types'
import { doActionWithError } from '../../../request/action'
import { formalizeAppID } from '../../../appuser/app/local'

export const useTaskConfigStore = defineStore('TaskConfigs', {
  state: () => ({
  ...
  actions: {
    ...
    userGetTasks (req: UserGetTasksRequest, done: (error: boolean, rows?: Array<TaskConfig>) => void) {
      doActionWithError<UserGetTasksRequest, UserGetTasksResponse>(
        API.USER_GET_TASKS,
        req,
        req.Message,
        (resp: UserGetTasksResponse): void => {
          this.addTaskConfigs(undefined, resp.Infos)
          done(false, resp.Infos)
        }, () => {
          done(true)
