...
export const useRecommendStore = defineStore('app-good-recommends', {
  ...
  getters: {
    ...
    recommends (): (appID?: string) => Array<Recommend> {
      return (appID?: string) => {
        appID = formalizeAppID(appID)
  ...
  actions: {
    addRecommends (goods: Array<Recommend>) {
      goods.forEach((recommend) => {
        if (!recommend) return
        const _goods = this.Recommends.get(recommend.AppID) as Array<Recommend> || []
        const index = _goods.findIndex((el) => el.EntID === recommend.EntID)
        _goods.splice(index >= 0 ? index : 0, index >= 0 ? 1 : 0, recommend)
        this.Recommends.set(recommend.AppID, _goods)
      })
    },
    _deleteRecommend (recommend: Recommend) {
